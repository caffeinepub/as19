...
export default function MemoriesStorageMonitor() {
  const { data: metrics } = useGetStorageMetrics();
  const { language } = useLanguage();
  const t = useTranslations(language);

  if (!metrics?.memories) return null;

  const { used, limit, percentage } = metrics.memories;
    ...
    <Card className="glass-effect dark:glass-effect-dark">
      <CardHeader>
        <div className="flex items-center gap-3">
          ...
          <div>
            <CardTitle className="text-base">{t('storage.memories')}</CardTitle>
