...
export default function StorageMonitor() {
  ...
  const photoPercentage = Number(metrics.photos.percentage);

  const showWarning = photoPercentage >= STORAGE_WARNING_PERCENTAGE;
    ...
    <Card className="border-rose-200 dark:border-rose-800">
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          <img src="/assets/generated/storage-heart-meter-transparent.dim_128x128.png" alt="" className="w-5 h-5" />
          फ़ोटो स्टोरेज
        </CardTitle>
        <CardDescription className="flex items-center gap-1.5">
          <Server className="w-3 h-3" />
          आवंटित: {formatSize(photoLimit)} (~6 GB मल्टी-कैनिस्टर)
        </CardDescription>
      </CardHeader>
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between text-sm">
            ...
            <span className="font-medium">
              {formatSize(photoUsed)} / {formatSize(photoLimit)}
            </span>
          </div>
          <Progress 
            value={Math.min(photoPercentage, 100)} 
            className={`h-2 ${showWarning ? '[&>div]:bg-rose-500 dark:[&>div]:bg-rose-600' : '[&>div]:bg-rose-400 dark:[&>div]:bg-rose-500'}`}
          />
          <p className="text-xs text-muted-foreground text-right">
            {photoPercentage.toFixed(1)}% उपयोग किया गया
          </p>
        </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="space-y-1 flex-1">
                <AlertDescription className="text-rose-800 dark:text-rose-200 font-medium">
                  {photoPercentage >= 100 ? 'फ़ोटो स्टोरेज पूर्ण हो गया है' : 'फ़ोटो स्टोरेज सीमा के करीब पहुंच रहे हैं'}
                </AlertDescription>
                <p className="text-sm text-rose-700 dark:text-rose-300">
                  कृपया पुरानी फ़ोटो हटाएं या स्टोरेज प्रबंधित करें
                </p>
              </div>
            </div>
          </Alert>
